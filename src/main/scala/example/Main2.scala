...
import org.http4s.{Headers, HttpService, Request}
...
object Main2 extends StreamApp[IO] with Logging {
  val port = 9000
        ...
        "/" -> HttpService[IO] {
          case req@GET -> Root / "foo" =>
            for {
              ...
              res <- client.expect[String](Request[IO](method = GET, uri = uri(s"http://localhost:$port/bar"), headers = Headers.empty))
